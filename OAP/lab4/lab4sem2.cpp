...
void ifile()// Функция чтения из файла
{
	string strInf;
...
void del()// Функция удаления строки
{
	int d;
	...
	if (d == 1)
	{
		for (int de1 = 0; de1 < current_size - 1; de1++)
		{
	...
	if (d != 32 and d != 1)
	{
		for (int de1 = (d - 1); de1 < current_size - 1; de1++)
		{
			list_of_ex[de1] = list_of_ex[de1 + 1];
		}
		current_size = current_size - 1;
...
void outputOneStructure(table oneex) // Функция вывода одной структуры
{
	cout << "название ";
